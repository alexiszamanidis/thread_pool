...
struct generic_queue *initialize_generic_queue(size_t size) {
    struct generic_queue *new_queue = my_malloc(struct generic_queue,1);
...
void push_generic_queue(struct generic_queue **queue, void *data) {
    error_handler(queue == NULL,"queue is NULL");

    // allocate queue node
    struct generic_queue_node *new_queue_node = my_malloc(struct generic_queue_node,1);
...
void *pop_generic_queue(struct generic_queue **queue) {
    error_handler(queue == NULL,"queue is NULL");

    struct generic_queue_node *temp_queue_node = (*queue)->head;
...
void free_generic_queue(struct generic_queue **queue) {
...
void free_generic_queue_node(struct generic_queue_node **generic_queue_node) {
    if( *generic_queue_node == NULL )
        return;

    free((*generic_queue_node)->data);
    free((*generic_queue_node));
}
